...
![System Architecture Diagram: React/FastAPI Orchestration](docs/system-architecture.png)

> **React/Vite UI** communicates with the **FastAPI Backend** via `POST /run` to submit tasks and `GET /task/{id}` to poll for updates. The **Orchestrator** drives agents in sequence (Planner → Researcher ×3 via `asyncio.gather()` → Writer → Reviewer) and persists results in an in-memory Task Store. All LLM calls are routed through a shared **LLM Client** supporting Gemini, OpenAI, and Ollama with exponential backoff.
...
![Pipeline Visualizer: Stage Flow Diagram](docs/pipeline-visualizer.png)

> The pipeline progresses through four stages: **planning → researching → writing → reviewing**. If the Reviewer requests revisions, the pipeline loops back to the Writer (up to 3 times). On approval (or max revisions), the task reaches **completed**. The entire pipeline can also repeat for new iterations.
